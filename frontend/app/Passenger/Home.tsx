...
const Home = () => {
  ...
  const handleDropdownChange = (item: DropdownOption) => {
    ...
    // Define valid paths
    const validPaths = ['/Passenger/Home', '/Passenger/PassengerProfil', '/Passenger/Actualities'];
  
    // Dynamically create the path
    const selectedPath = `/Passenger/${item.value}`;
        ...
        {/*     <DataTable.Title>Available</DataTable.Title> */}
          ...
          {currentItems.map((ride, index) => (
            ...
            {/*   <DataTable.Cell>{ride.available}</DataTable.Cell> */}
              <DataTable.Cell>
                <TouchableOpacity onPress={() => router.push(`/Passenger/${ride._id}`)}>
